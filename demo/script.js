...
function checkAuth() {
    ...
    if (!adminToken) {
        // Создаем демо-токен и входим автоматически
    ...
    // Всегда показываем админ-контент для демо
...
function logout() {
    ...
    if (userRole === 'demo') {
        // Для демо-режима очищаем все токены и перезагружаем страницу
    ...
    if (!document.getElementById('product-list')) {
        productsContainer.innerHTML = `
            <div class="flex items-center mb-4">
                ...
                <button class="ml-2 bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-full transition" onclick="navigateToCreatePage()"><i class="fas fa-plus"></i></button>
            </div>
    ...
    try {
        ...
        products.forEach(product => {
            ...
            productCard.innerHTML = `
                ...
                <div class="flex items-center">
                    <button class="text-blue-500 hover:text-blue-700 p-2 lg:p-3" onclick="event.stopPropagation(); navigateToEditPage('${product.id}')" title="${userRole === 'demo' ? 'Просмотр товара (Demo режим)' : 'Редактировать товар'}">
                        <i class="fas fa-edit lg:text-lg"></i>
                    </button>
                </div>
            `;
...
function navigateToEditPage(productId) {
    window.location.href = `/demo/edit.html?id=${productId}`;
}

function navigateToCreatePage() {
    window.location.href = '/demo/edit.html';
}
...
function setupStatusFilterScroll() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (scanBtn) {
            scanBtn.addEventListener('click', startSaleScan);
        }
        
        const demoLoginBtn = document.getElementById('demo-login-btn');
        if (demoLoginBtn) {
            demoLoginBtn.addEventListener('click', handleLogin);
        }
